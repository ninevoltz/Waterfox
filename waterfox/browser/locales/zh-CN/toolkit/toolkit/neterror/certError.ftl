...
cert-error-mitm-mozilla = { -brand-short-name } 由非营利的 BrowserWorks 提供支持。BrowserWorks 管理一组完全开放的数字证书认证机构（CA）存储库。该存储库帮助确保这些数字证书认证机构遵循最佳实践，以保障用户的安全。

cert-error-mitm-connection = { -brand-short-name } 使用 BrowserWorks 的数字证书认证机构存储库来验证连接是否安全，而非用户操作系统所提供的证书库。因此，如果您的防病毒软件或网络使用不在 BrowserWorks 数字证书认证机构列表中的机构所签发的证书来拦截网络流量，该连接被视为不安全。
